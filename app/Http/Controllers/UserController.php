...
class UserController extends Controller
{
    ...
    public function updateViva(Request $request) {
        Viva::where('Pr_Key',$request->prKey)
            ->update(['Pr_Name'=>$request->name,'Anee' => $request->year,'Fst_Student' => $request->firstStudent,'Sst_Student'=>$request->secondStudent,
            'Tst_Student'=>$request->thirdStudent,'Supervisor'=>$request->supervisor,'Examiner'=>$request->examiner,'President'=>$request->president,
            'Sp_Mark'=>$request->spMark,'Ex_Mark'=>$request->exMark,'Pr_Mark'=>$request->prMark,'Final_Mark'=>$request->vivaMark,]);
            return response()->json([
